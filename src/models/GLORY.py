...
from models.component.nce_loss import NCELoss, UpgradeNCELoss, EnhancedNCELossRanking, PairwiseRankingLoss
...
class GLORY(nn.Module):
    def __init__(self, cfg, glove_emb=None, entity_emb=None):
        ...
        self.is_ClickNet = cfg.model.is_ClickNet
        self.is_new_loss = cfg.model.is_new_loss
        self.is_combined = cfg.model.is_combined
        ...
        # click prediction
        # is_ClickNet and is_new_loss defined from configs\model\default.yaml
        if self.is_ClickNet:
             # self.click_predictor = RNNClickNet(embedding_dim=400)  # 400 = cand_final_emb.size(-1)
            self.click_predictor = ClickNet(embedding_dim=400)  # 400 = cand_final_emb.size(-1)
            # self.click_predictor = ClickNetDotProduct()  # 400 = cand_final_emb.size(-1)
        else:
            self.click_predictor = DotProduct()

        if self.is_new_loss:
            if self.is_combined:
                self.loss_fn = EnhancedNCELossRanking()
            else:
    ...
    def forward(self, subgraph, mapping_idx, candidate_news, candidate_entity, entity_mask, label=None):
        ...
        # print(f'cand_final_emb: {cand_final_emb}')
